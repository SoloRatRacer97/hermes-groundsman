#!/usr/bin/env node
/**
 * Hermes V2.4 — 200-Message Stress Test
...
function gradeResponse(response, lead, isFollowUp) {
  ...
  // Strip leading greetings before counting — greetings are not content
  let contentText = text.replace(/\[TRANSFER\]/gi, '').trim();
  // Remove leading greeting patterns: "Hey Name.", "Hi Name,", "Hey Name," etc.
  contentText = contentText.replace(/^(hey|hi|hello|yo)\s+[a-z]+[.,]\s*/i, '');
  // Count sentences by splitting on sentence-ending punctuation followed by space or end
  const sentenceParts = contentText.split(/(?<=[.!?])\s+|(?<=[.!?])$/).filter(s => s.trim().length > 3);
  const hasTransfer = /\[TRANSFER\]/i.test(text);
  const maxSentences = hasTransfer ? 3 : 2;
  grades.sentenceCount = sentenceParts.length <= maxSentences ? 'PASS' : 'FAIL';
  grades.sentenceCountDetail = `${sentenceParts.length}/${maxSentences}`;
...
async function main() {
  ...
  console.log(`\n🚀 Hermes V2.4 — 200-Message Stress Test (${MODEL})`);
  ...
  let md = `# Hermes V2.4 — 200-Message Stress Test Results\n\n`;
  ...
  fs.writeFileSync(path.join(outputDir, 'hermes-v2.4-200-results.md'), md);

  // ─── Diagnosis ───
  let diag = `# Hermes V2.4 — 200-Message Stress Test Diagnosis\n\n`;
  ...
  if (sortedCriteria.length === 0) {
    diag += `No failures detected. Framework V2.4 is performing at target.\n`;
  } else {
  ...
  fs.writeFileSync(path.join(outputDir, 'hermes-v2.4-200-diagnosis.md'), diag);

  console.log(`✅ Results: output/hermes-v2.4-200-results.md`);
  console.log(`✅ Diagnosis: output/hermes-v2.4-200-diagnosis.md`);
}

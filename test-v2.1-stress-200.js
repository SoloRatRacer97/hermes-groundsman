#!/usr/bin/env node
/**
 * Hermes V2.1 — 200-Message Stress Test
...
async function main() {
  ...
  console.log(`\n🚀 Hermes V2.1 — 200-Message Stress Test (${MODEL})`);
  ...
  fs.writeFileSync(path.join(outputDir, 'hermes-v2.1-200-results.md'), md);

  // ─── Diagnosis ───
  let diag = `# Hermes V2.1 — 200-Message Stress Test Diagnosis\n\n`;
  ...
  if (sortedCriteria.length === 0) {
    diag += `No failures detected. Framework V2.1 is performing at target.\n`;
  } else {
  ...
  fs.writeFileSync(path.join(outputDir, 'hermes-v2.1-200-diagnosis.md'), diag);
